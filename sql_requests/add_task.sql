...
SELECT tbl1.id,
	   var1, var2,
	   CASE 
	   		WHEN tbl1.valid_from_dttm > tbl2.valid_from_dttm::timestamp THEN tbl1.valid_from_dttm
			ELSE tbl2.valid_from_dttm
	   END,
	   CASE 
	   		WHEN tbl1.valid_to_dttm < tbl2.valid_to_dttm THEN tbl1.valid_to_dttm
			ELSE tbl2.valid_to_dttm
	   END
--SELECT *
FROM Table1 tbl1 JOIN Table2 tbl2 ON 
	tbl1.id = tbl2.id AND
